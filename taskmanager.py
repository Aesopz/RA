...
from routing import plan_route, euclidean_distance
...
class TaskManager:
    ...
    def generate_random_task(self, num_locations: int = 1) -> Optional[Task]:
        ...
        # 隨機選擇一個貨架位置
        shelf_location = random.choice(self.shelf_coords)

        task = {
            "task_id": self.next_task_id,
            "shelf_location": shelf_location, # 簡化為單一地點
        }
        self.task_queue.append(task)
        self.next_task_id += 1
        print(f"✨ 已生成新任務 {task['task_id']}，目標貨架: {shelf_location}")
        return task
    ...
    def assign_pending_tasks(self, robots: Dict[str, 'Robot'], warehouse_matrix: np.ndarray, forbidden_cells_for_tasks: Optional[Set[Coord]] = None):
        ...
        for task in self.task_queue:
            ...
            if available_robots:
                ...
                target_pos = task["shelf_location"]
