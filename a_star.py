...
DEFAULT_COSTS: Dict[int, float] = {
    1: 1.0,   # turn road
    2: 1.2,   # sub-road  (稍高成本 → 模擬巷道較窄或擁擠)
    3: 1.0,   # main road
    4: 1.0,   # picking station
    5: 1.0,   # charge station
    6: 1.0,   # queue charge
    7: 1.0,   # charge leave
    8: 1.0,   # queue pick
    # 0 不列入：代表障礙／貨架 block
...
if __name__ == "__main__":
    from layout import build_layout  # 直接重用你的 layout.py
    wl = build_layout()
    grid = wl.layout_matrix

    start = (13, 2)   # 或其他在 grid 中合法的位置
    goal = (0, 7)     # 確保在 layout 範圍內
    # 任選一個巷道轉彎
